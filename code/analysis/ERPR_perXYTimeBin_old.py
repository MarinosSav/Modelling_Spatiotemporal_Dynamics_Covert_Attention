...
erprAmp2DMapPerTimeBin = np.zeros(
    (np.max(nYBinsArr), np.max(nXBinsArr), nTimeBins, len(nYBinsArr))
)
...
erprAmp2DMapPerTimeBin_ave = np.median(erprAmp2DMapPerTimeBin, axis=3)
...
for bT in range(len(binCentersT)):
    ...
    plt.subplot(5, 1, countBT + 1)
    plt.imshow(erprAmp2DMapPerTimeBin_ave[:, :, countBT], cmap="coolwarm_r", vmin=-160, vmax=60)
